...
- `dashboard/app.py` — Streamlit front end (Phase 4, done). Reads `outputs/dashboard/`, `config.yaml`, `current_holdings.csv`; basket sliders rewrite config.yaml via ruamel.yaml (comments preserved); never runs the pipeline itself. Exception (Phase 5): the dashboard may call `engine/analyzer.py` and `engine.report.rebalance_recommendation` directly for read only calculations, and nothing more. Run: `streamlit run dashboard/app.py`. Deps: `dashboard/requirements.txt`.
